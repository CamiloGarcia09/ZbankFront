...
import DatosUsuario from "../Form/DatosUsuario";
import { postModel } from "../../Models/postModel";
import { enviarDatos, obtenerDatosDivisas } from '../../Services/ApiService'
import "../SignIn/FormSingIn.css"
...
function FormSingIn() {
  ...
  const [data, setDataDivisa] = useState([]);
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      setPostStatus(error)
      alert(error.response.data.mensajes[0])
      setDisableButton(false)
    }
  }
  ...
  return (
    <Card className="container">
      <Form >
        <Form.Text
          className="container-title mb-6"> Registro Usuario ZBank </Form.Text>
          ...
          <Col md={12}>
            {
              data && (
                <><Form.Text
                  className="form-text-content"> Seleccione su divisa </Form.Text>
                  <Form.Select
                    id="divisa"
        ...
        <Row className="mb-3">
          <Col className="distribution">
            <Button
              className={`btn ${disableButton ? 'btn-disabled' : ''}`}
              onClick={handlePost}
              disabled={disableButton}> Registrar
            </Button>
          </Col>
        </Row>
      </Form>
